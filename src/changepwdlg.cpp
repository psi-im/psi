...
ChangePasswordDlg::~ChangePasswordDlg() { pa->dialogUnregister(this); }
...
void ChangePasswordDlg::apply()
{
    ...
    if (ui_.le_pwcur->text().isEmpty() || ui_.le_pwnew->text().isEmpty() || ui_.le_pwver->text().isEmpty()) {
        QMessageBox::information(this, tr("Error"),
                                 tr("You must fill out the fields properly before you can proceed."));
        return;
    }
    ...
    if (ui_.le_pwnew->text() != ui_.le_pwver->text()) {
        QMessageBox::information(this, tr("Error"),
                                 tr("New password and confirmation do not match.  Please enter them again."));
...
void ChangePasswordDlg::finished()
{
    ...
    if (ok) {
        ...
        QMessageBox::critical(this, tr("Error"),
                              QString(tr("There was an error when trying to set the password.\nReason: %1"))
...
void ChangePasswordDlg::blockWidgets() { setWidgetsEnabled(false); }

void ChangePasswordDlg::restoreWidgets() { setWidgetsEnabled(true); }
