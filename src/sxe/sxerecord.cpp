...
QDomNode SxeRecord::node() const { return node_; }
...
void SxeRecord::processInOrderRecordEdit(const SxeRecordEdit *edit)
{
    ...
    foreach (SxeRecordEdit::Key key, edit->keys()) {
        ...
        if (key == SxeRecordEdit::Parent && edit->value(key) != parent()) {
        ...
        } else if (key == SxeRecordEdit::ProcessingInstructionTarget
                   && edit->value(key) != processingInstructionTarget()) {
            ...
            if (keys.contains(SxeRecordEdit::ReplaceFrom) && keys.contains(SxeRecordEdit::ReplaceN)) {
                ...
                if (ok1 && ok2 && from >= 0 && n >= 0 && from + n <= data_.length()) {
                    ...
                    if (!ok1)
                        qDebug("Could not convert 'replacefrom' = '%s' to int.",
                               qPrintable(edit->value(SxeRecordEdit::ReplaceFrom)));
                    if (!ok2)
                        qDebug("Could not convert 'replacen' = '%s' to int.",
                               qPrintable(edit->value(SxeRecordEdit::ReplaceN)));
                    if (from < 0)
                        qDebug("'replacefrom' = '%s' is negative.",
                               qPrintable(edit->value(SxeRecordEdit::ReplaceFrom)));
                    if (n < 0)
...
void SxeRecord::updateNode(bool remote)
{
    ...
    if (identifier_ != lastIdentifier_) {
        ...
        if ((node_.isElement() || node_.isAttr()) && node_.nodeName() != name()) {
...
QString SxeRecord::rid() const { return rid_; };

QString SxeRecord::parent() const { return parent_; }

double SxeRecord::primaryWeight() const { return primaryWeight_; }

int SxeRecord::version() const { return edits_.size() - 1; }

QString SxeRecord::name() const { return identifier_; }
...
QString SxeRecord::chdata() const { return data_; }

QString SxeRecord::processingInstructionTarget() const { return identifier_; }

QString SxeRecord::processingInstructionData() const { return data_; }
...
bool SxeRecord::operator==(const SxeRecord &other) const { return rid() == other.rid(); }
...
bool SxeRecord::operator>(const SxeRecord &other) const { return !(*this == other || *this < other); }
