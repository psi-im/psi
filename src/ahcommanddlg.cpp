...
static bool operator<(const AHCommandItem &ci1, const AHCommandItem &ci2) { return ci1.name < ci2.name; }
...
public:
    AHCExecuteTaskWrapper(PsiCon *psi, AHCExecuteTask *task) : QObject(task), psi(psi), task(task)
    {
...
AHCommandDlg::AHCommandDlg(PsiAccount *pa, const Jid &receiver) : QDialog(nullptr), pa_(pa), receiver_(receiver)
{
    ui_.setupUi(this);
