...
MainWindow::MainWindow(QWidget *parent, const char *name) : Q3MainWindow(parent, name)
{
    ...
    game->insertItem(QPixmap((const char **)new_game), tr("New"), this, SLOT(newGame()), Qt::CTRL + Qt::Key_N);
    id = game->insertItem(xpm, tr("Save image"), this, SLOT(saveImage()), Qt::CTRL + Qt::Key_S);
    game->setItemEnabled(id, false);
    game->insertSeparator();
    game->insertItem(QPixmap((const char **)quit_xpm), tr("Quit"), qApp, SLOT(quit()), Qt::CTRL + Qt::Key_Q);
    ...
    QObject::connect(sock, SIGNAL(acceptConnection(int)), this, SLOT(newGame(int)));
    QObject::connect(wrk, SIGNAL(windowActivated(QWidget *)), this, SLOT(activated(QWidget *)));
}
...
void MainWindow::showStatus(const QString &txt) { statusBar()->message(txt); }
...
void MainWindow::newGame(int sock)
{
    GameBoard *brd;
...
void MainWindow::activated(QWidget *w)
{
...
SelectGame::SelectGame(QWidget *parent, const char *name) : QDialog(parent, name)
{

    setCaption(tr("New game with..."));
    l1  = new QLabel(tr("To play with "), this);
    hst = new QComboBox(true, this);
    hst->setValidator(new QRegExpValidator(QRegExp("([a-zA-Z0-9]*\\.)*[a-zA-Z]"), hst));
    ...
    QObject::connect(hst, SIGNAL(textChanged(const QString &)), this, SLOT(checkParams(const QString &)));
...
void SelectGame::resizeEvent(QResizeEvent *e)
{
    QFontMetrics fm(font());
    int          w = e->size().width(), h = e->size().height(), fh = fm.lineSpacing() + 2, bl;
    ...
    bg->setGeometry(wg->x(), wg->y() + wg->height(), wg->width(), wg->height());
    box->move(btn->x(), btn->y() + btn->height());
    box->resize(w, h - box->y());
    bl = box->width() / 5;
    Ok->move(bl, 4);
    Ok->resize(bl, box->height() - Ok->y() * 2);
    Cancel->setGeometry(Ok->x() + Ok->width() + bl, Ok->y(), Ok->width(), Ok->height());
}

void SelectGame::checkParams()
{
    bool res;

    res = !hst->currentText().isEmpty() && (bg->isChecked() || wg->isChecked());
    Ok->setEnabled(res);
}

void SelectGame::checkParams(const QString &) { checkParams(); }

QString SelectGame::host()
{
...
void SelectGame::setHosts(const QStringList &h) { hst->insertStringList(h); }
