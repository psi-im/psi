...
SnarlPlugin::SnarlPlugin() : PsiPlugin()
{
    ...
    if (major == 0 && minor == 0)
        QMessageBox::information(0, "Snarl",
                                 QString("Snarl is not running, so notifications are disabled until it is started"));
}

SnarlPlugin::~SnarlPlugin() { delete snarl_; }

QString SnarlPlugin::name() const { return "Snarl Plugin"; }

QString SnarlPlugin::shortName() const { return "snarl"; }
