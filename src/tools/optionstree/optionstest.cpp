...
int main (int argc, char const* argv[])
{
	...
	tree.saveOptions("options.xml","OptionsTest","0.1");
	
	OptionsTree tree2;
	tree2.loadOptions("options.xml","OptionsTest","0.1");
	foreach(QString name, tree2.allOptionNames())
	{
		qWarning(qPrintable(name));
		qWarning(qPrintable(tree2.getOption(name).toString()));
	}
	tree2.saveOptions("options2.xml","OptionsTest","0.1");
	
	OptionsTree tree3;
	tree3.loadOptions("options.xml","OptionsTest","0.1");
	tree3.saveOptions("options3.xml","OptionsTest","0.1");
	return 0;
}
