...
void GoogleSessionListener::signalingReady() { manager_->session_manager_->OnSignalingReady(); }
...
GoogleFileTransfer::GoogleFileTransfer(cricket::FileShareSession *s, GoogleFTManager *manager) :
    session_(s), manager_(manager)
{
...
XMPP::Jid GoogleFileTransfer::peer() const { return Jid(session_->jid().BareJid().Str().c_str()); }

QString GoogleFileTransfer::fileName() const { return description(); }
...
QString GoogleFileTransfer::description() const
{
    ...
    if (session_->manifest()->size() == 1)
        ...
        description = QString("%1 files and %2 directories")
                          .arg(session_->manifest()->GetFileCount())
                          .arg(session_->manifest()->GetFolderCount());
...
void GoogleFileTransfer::accept(qlonglong, qlonglong) { session_->Accept(); }

void GoogleFileTransfer::reject() { session_->Decline(); }

void GoogleFileTransfer::cancel() { session_->Cancel(); }
...
void GoogleFTManager::initialize()
{
    ...
    file_share_session_client_->SignalFileShareSessionCreate.connect(listener_,
                                                                     &GoogleSessionListener::fileShareSessionCreated);
