...
AccountModifyDlg::AccountModifyDlg(PsiCon *_psi, QWidget *parent) : QDialog(parent)
{
    acc.name = "";
    setupUi(this);
    setModal(true);
...
AccountModifyDlg::AccountModifyDlg(PsiAccount *_pa, QWidget *parent) : QDialog(parent)
{
    setupUi(this);
    setModal(false);
...
void AccountModifyDlg::init()
{
    ...
    if (pa) {
        connect(pa->privacyManager(), SIGNAL(defaultListAvailable(const PrivacyList &)),
                SLOT(updateBlockedContacts(const PrivacyList &)));
    ...
    if (!PsiOptions::instance()->getOption("options.ui.account.security.show").toBool()
        && !PsiOptions::instance()->getOption("options.ui.account.keepalive").toBool()
        && !PsiOptions::instance()->getOption("options.ui.account.sm.show").toBool()
        && !PsiOptions::instance()->getOption("options.ui.account.manual-host").toBool()
        && !PsiOptions::instance()->getOption("options.ui.account.proxy.show").toBool()) {
    ...
    if (!PsiOptions::instance()->getOption("options.ui.account.resource").toBool()
        && !PsiOptions::instance()->getOption("options.ui.account.priority").toBool()
        && !PsiOptions::instance()->getOption("options.ui.account.data-proxy").toBool()) {
...
void AccountModifyDlg::save()
{
    /*if(pa && le_name->text().isEmpty()) {
        QMessageBox::information(this, tr("Error"), tr("You must specify a name for the account before you may save
    it.")); return;
    }*/
    ...
    if (newJid.node().isEmpty() || newJid.domain().isEmpty()) {
        if (!PsiOptions::instance()->getOption("options.account.domain").toString().isEmpty()) {
            ...
            QMessageBox::information(this, tr("Error"),
                                     tr("<i>XMPP Address</i> must be specified in the format <i>user@host</i>."));
        }
        return;
    }
    ...
    if (pa) {
        ...
        if (pa->isActive()) {
            QMessageBox messageBox(
                QMessageBox::Information, tr("Warning"),
...
void AccountModifyDlg::tabChanged(int) { updatePrivacyTab(); }
...
void AccountModifyDlg::addBlockClicked()
{
    ...
    bool    ok;
    QString input
        = QInputDialog::getText(nullptr, tr("Block contact"), tr("Enter the XMPP Address of the contact to block:"),
                                QLineEdit::Normal, "", &ok);
    Jid jid(input);
    if (ok && !jid.isEmpty()) {
...
void AccountModifyDlg::setPrivacyTabEnabled(bool b) { ws_privacy->setCurrentWidget(b ? pg_privacy : pg_privacyStatus); }
...
void AccountModifyDlg::resourceCbChanged(int index) { le_resource->setEnabled(index == 0); }

void AccountModifyDlg::priorityCbChanged(int index) { sb_priority->setEnabled(index == 0); }
