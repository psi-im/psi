...
static XMPP::Jid processJid(const XMPP::Jid &jid) { return jid.withResource(""); }

static bool privacyListItemForJid(const PrivacyListItem &item, const Jid &jid)
{
    return item.type() == PrivacyListItem::JidType && processJid(item.value()) == processJid(jid);
}
...
public:
    ...
    const QStringList &lists() { return lists_; }

    const QString &defaultList() { return default_; }

    const QString &activeList() { return active_; }
};
...
public:
    SetPrivacyListsTask(Task *parent) :
        Task(parent), changeDefault_(false), changeActive_(false), changeList_(false), list_("")
    {
    }
    ...
    QString name() { return list_.name(); }
};
...
public:
    ...
    const PrivacyList &list() { return list_; }
};

// -----------------------------------------------------------------------------

PsiPrivacyManager::PsiPrivacyManager(PsiAccount *account, XMPP::Task *rootTask) :
    rootTask_(rootTask), getDefault_waiting_(false), block_waiting_(false), account_(account), accountAvailable_(false),
    isAvailable_(false)
{
    ...
    connect(this, SIGNAL(listsReceived(const QString &, const QString &, const QStringList &)),
            SLOT(newListsReceived(const QString &, const QString &, const QStringList &)));
...
void PsiPrivacyManager::block(const QString &target)
{
    ...
    if (!block_waiting_) {
        block_waiting_ = true;
        connect(this, SIGNAL(defaultListAvailable(const PrivacyList &)),
                SLOT(block_getDefaultList_success(const PrivacyList &)));
...
{
    PrivacyList l = l_;
    disconnect(this, SIGNAL(defaultListAvailable(const PrivacyList &)), this,
               SLOT(block_getDefault_success(const PrivacyList &)));
...
{
    connect(this, SIGNAL(listsReceived(const QString &, const QString &, const QStringList &)),
            SLOT(getDefault_listsReceived(const QString &, const QString &, const QStringList &)));
...
{
    disconnect(this, SIGNAL(listsReceived(const QString &, const QString &, const QStringList &)), this,
               SLOT(getDefault_listsReceived(const QString &, const QString &, const QStringList &)));
...
void PsiPrivacyManager::getDefault_listReceived(const PrivacyList &l)
{
    if (l.name() == getDefault_default_ && getDefault_waiting_) {
        disconnect(this, SIGNAL(listReceived(const PrivacyList &)), this,
                   SLOT(getDefault_listReceived(const PrivacyList &)));
...
void PsiPrivacyManager::getDefault_listError() { emit defaultListError(); }
...
bool PsiPrivacyManager::isAvailable() const { return isAvailable_; }
...
void PsiPrivacyManager::newListsReceived(const QString &defaultList, const QString &activeList,
...
void PsiPrivacyManager::newListsError() { setIsAvailable(false); }

void PsiPrivacyManager::newChangeDefaultList_success() {}

void PsiPrivacyManager::newChangeDefaultList_error() { qWarning("YaPrivacyManager::changeDefaultList_error()"); }

void PsiPrivacyManager::newChangeActiveList_success() {}

void PsiPrivacyManager::newChangeActiveList_error() { qWarning("YaPrivacyManager::changeActiveList_error()"); }
...
bool PsiPrivacyManager::isAuthorized(const XMPP::Jid &jid) const
{
    ...
    return u && (u->subscription().type() == Subscription::Both || u->subscription().type() == Subscription::From);
}
