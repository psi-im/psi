...
PrivacyList::PrivacyList(const QDomElement &e) { fromXml(e); }
...
void PrivacyList::appendItem(const PrivacyListItem &item) { insertItem(items_.count(), item); }
...
bool PrivacyList::onlyBlockItems() const
{
    ...
    for (it = items_.begin(); it != items_.end() && allBlocked; ++it) {
        if ((*it).type() == PrivacyListItem::FallthroughType && (*it).action() == PrivacyListItem::Allow
            && (*it).all()) {
            fallThrough = true;
        } else if ((*it).isBlock()) {
            if (fallThrough)
