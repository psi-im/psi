...
public:
	...
	// ###cuda
	bool needDisclaimer(const Jid &j);
	void extendDisclaimerTime(const Jid &j);
	QString disclaimer() const;
	QList<Jid> servicesList() const;
	QList<ChatDlg*> getChats();
	QList<GCMainDlg*> getGroupChats();
	void cpUpdate_pub(const UserListItem &u);
	void checkUpgrade();
...
public slots:
	...
	void doGotoWebPass();
	void doGotoWebHistory();

	void doMultiInvite(const QList<XMPP::Jid> &list, bool groupchat, const XMPP::Jid &jid);
