unix {
	...
	valgrind.commands = valgrind $$VALGRIND_OPTIONS $(TARGET) | grep -E '==[0-9]+=='

	# valgrind_supp target (generate suppressions)
	QMAKE_EXTRA_TARGETS += valgrind_supp
	valgrind_supp.depends = $(TARGET)
	valgrind_supp.commands = valgrind $$VALGRIND_OPTIONS --gen-suppressions=all $(TARGET)

	# callgrind profiling
	QMAKE_EXTRA_TARGETS += callgrind
	callgrind.depends = $(TARGET)
	callgrind.commands = valgrind --tool=callgrind --dump-instr=yes --collect-jumps=yes $(TARGET)
}
