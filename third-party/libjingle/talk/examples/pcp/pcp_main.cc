...
#include "talk/p2p/client/sessionclienttask.h"
...
buzz::Jid gUserJid;
InsecurePasswordImpl gUserPass;
std::string gXmppHost = "talk.google.com";
int gXmppPort = 5222;
bool gXmppUseTls = true;
...
void Usage() {
  std::cerr << "usage: pcp [options] <jid>                             (server mode)" << std::endl;
  std::cerr << "usage: pcp [options] <jid> [<jid>:]<src> [<jid>:]<dst> (client mode)" << std::endl;
...
void ParseFileArg(const char* arg, buzz::Jid* jid, std::string* file) {
  ...
  if (!sep) {
    *file = arg;
    ...
    if (jid_arg.IsBare())
      jid_arg = buzz::Jid(jid_arg.node(), jid_arg.domain(), "pcp");
    *jid = jid_arg;
...
buzz::XmppClientSettings LoginSettings() {
  ...
  xcs.set_pass(buzz::XmppPassword(gUserPass));
  SocketAddress server(gXmppHost, gXmppPort);
...
public:
  ...
  void Serve(TunnelSessionClient* client) {
  ...
  void OnStateChange(buzz::XmppEngine::State state) {
    ...
    case buzz::XmppEngine::STATE_OPEN:
      ...
      Thread::Current()->Post(NULL, MSG_LOGIN_COMPLETE);
      break;
    case buzz::XmppEngine::STATE_CLOSED:
      std::cout << "logged out..." << std::endl;
      Thread::Current()->Post(NULL, MSG_LOGIN_FAILED);
      break;
    }
  }

  void OnSendStanza(cricket::SessionClient* client,
                    const buzz::XmlElement* stanza,
                    bool response) {
    SendStanza(stanza);
  }

  void OnIncomingTunnel(TunnelSessionClient* client, buzz::Jid jid,
    std::string description, Session* session) {
    ...
    if (!ProcessStream(client->AcceptTunnel(session), filename, send))
      Thread::Current()->Post(NULL, MSG_DONE);
  }

  bool ProcessStream(StreamInterface* stream, const std::string& filename,
                     bool send) {
    ASSERT(file_.get() == NULL);
    sending_ = send;
    file_.reset(new FileStream);
    buffer_len_ = 0;
    if (!file_->Open(filename.c_str(), sending_ ? "rb" : "wb"))
      return false;
    stream->SignalEvent.connect(this, &CustomXmppPump::OnStreamEvent);
    if (stream->GetState() == SS_OPEN) {
      OnStreamEvent(stream, SE_OPEN | SE_READ | SE_WRITE, 0);
    }
    return true;
  }
  
  void OnStreamEvent(StreamInterface* stream, int events, int error) {
    if (events & SE_CLOSE) {
      if (error == 0) {
  ...
  void Cleanup(StreamInterface* stream, bool delay = false) {
    stream->Close();
    file_.reset();
    if (!server_)
      if (delay)
        Thread::Current()->PostDelayed(2000, NULL, MSG_DONE);
      else
        Thread::Current()->Post(NULL, MSG_DONE);
  }
...
int main(int argc, char **argv) {
  ...
  std::cout << "Directory: " << Traits<char>::string(path) << std::endl;

  buzz::Jid gSrcJid(gUserJid);
  buzz::Jid gDstJid(gUserJid);
  std::string gSrcFile;
  std::string gDstFile;
  ...
  {
    scoped_ptr<buzz::XmlElement> presence(
  ...
  InitRandom(user_jid_str.c_str(), user_jid_str.size());

  // Create the P2P session manager.
  NetworkManager network_manager;
  BasicPortAllocator allocator(&network_manager);
  SessionManager session_manager(&allocator);
  session_manager.SignalRequestSignaling.connect(&session_manager,
    &SessionManager::OnSignalingReady);
  TunnelSessionClient session_client(pump.client()->jid(), &session_manager);
  ...
  if (as_server) {
    ...
    std::string filename;
    bool sending;
    if (gSrcJid.BareEquals(gUserJid)) {
